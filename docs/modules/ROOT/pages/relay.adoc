...
[[eip1559-policy]]
==== EIP1559 Pricing
Specify if the transactions the Relayer is sending should be EIP1559 by default or not. This applies whenever the Relayer send a transaction with the <<relay-api-reference.adoc#send-transaction,`speed`>> parameter, or a non specified `gasPrice` or `maxFeePerGas`/`maxPriorityFeePerGas`. Note that this is only shown for EIP1559 compatible networks.
...
Specify if the transactions should be sent via private mempoool. This means that a transaction will not be publicly seen until its included in a block.
